...
import com.venusource.app.qywechat.api.WxCpAgentService;
...
public class WxCpAgentServiceImpl implements WxCpAgentService {
  ...
  public WxCpAgent get(Integer agentId) throws WxErrorException {
    ...
    String url = this.mainService.getWxCpConfigStorage().getWxworkServer() + "/cgi-bin/agent/get?agentid=" + agentId;
  ...
  public void set(WxCpAgent agentInfo) throws WxErrorException {
    String url = this.mainService.getWxCpConfigStorage().getWxworkServer() + "/cgi-bin/agent/set";
  ...
  public List<WxCpAgent> list() throws WxErrorException {
    String url = this.mainService.getWxCpConfigStorage().getWxworkServer() + "/cgi-bin/agent/list";
    String responseContent = this.mainService.get(url, null);
    JsonObject jsonObject = JSON_PARSER.parse(responseContent).getAsJsonObject();
    if (jsonObject.get("errcode").getAsInt() != 0) {
      throw new WxErrorException(WxError.fromJson(responseContent));
